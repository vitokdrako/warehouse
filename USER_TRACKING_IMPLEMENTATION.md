...
```bash
# 1. Отримати історію
curl "https://action-audit.preview.emergentagent.com/api/user-tracking/orders/7040/history"

# 2. Отримати нотатки
curl "https://action-audit.preview.emergentagent.com/api/user-tracking/orders/7040/notes"

# 3. Додати нотатку (з токеном)
curl -X POST "https://action-audit.preview.emergentagent.com/api/user-tracking/orders/7040/notes" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"note": "Тестова внутрішня нотатка"}'

# 4. Залогувати комплектацію
curl -X POST "https://action-audit.preview.emergentagent.com/api/user-tracking/orders/7040/pack-item" \
  -H "Authorization: Bearer YOUR_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "item_id": "test_123",
