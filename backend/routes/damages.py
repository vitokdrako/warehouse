...
def parse_damage(row):
    ...
    return {
        ...
        "severity": row[10],
        "source": row[11],
        "finance_status": row[13],
        "fulfillment_status": row[14],
        "claimed_total": float(row[15]) if row[15] else 0.0,
        "paid_total": float(row[16]) if row[16] else 0.0,
        "withheld_total": float(row[17]) if row[17] else 0.0,
        "deposit_available": float(row[18]) if row[18] else 0.0,
        "notes": row[19],
        "created_by": row[21],
        "assigned_to": row[22],
