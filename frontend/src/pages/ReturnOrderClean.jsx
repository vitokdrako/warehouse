...
/******************** modal finding ********************/
function FindingModal({open, item, onClose, onSave}){
  const [f,setF] = useState({category: item?.category || 'Меблі', kindCode:'', severity:'low', note:'', fee:0, photoName:''})
  
  useEffect(()=>{
    if(item && open){
      setF({category: item.category || 'Меблі', kindCode:'', severity:'low', note:'', fee:0, photoName:''})
    }
  },[item, open])
  ...
  const selectedCat = DAMAGE_RULES[f.category] || {groups:[]}
  const kinds = selectedCat.groups
  const selectedKind = kinds.find(k=>k.code===f.kindCode)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-xl rounded-2xl border border-slate-200 bg-white p-4 shadow-xl max-h-[90vh] overflow-auto">
        <div className="mb-3 flex items-center justify-between">
          <h3 className="text-base font-semibold">Пошкодження · {item.sku} · {item.name}</h3>
          <button onClick={onClose} className="text-slate-500 hover:text-slate-700">✕</button>
        </div>
        <div className="grid gap-3 text-sm">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <div className="mb-1 text-slate-500">Категорія</div>
              <select className="w-full rounded-xl border px-3 py-2" value={f.category} onChange={e=>setF({...f, category:e.target.value, kindCode:'', fee:0})}>
                {categories.map(c=> <option key={c} value={c}>{c}</option>)}
              </select>
            </div>
            <div>
              <div className="mb-1 text-slate-500">Тип</div>
              <select className="w-full rounded-xl border px-3 py-2" value={f.kindCode} onChange={e=>{
                const code=e.target.value; const k = kinds.find(x=>x.code===code); setF(prev=>({...prev, kindCode:code, fee: defaultFeeFor(k)}))
              }}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <div className="mb-1 text-slate-500">Рівень</div>
              <select className="w-full rounded-xl border px-3 py-2" value={f.severity} onChange={e=>setF({...f, severity:e.target.value})}>
            ...
            <div>
              <div className="mb-1 text-slate-500">Сума (грн)</div>
              <div className="flex gap-2">
                <input className="w-full rounded-xl border px-3 py-2" type="number" value={f.fee} onChange={e=>setF({...f, fee:Number(e.target.value)||0})} />
                <button className="rounded-lg border px-2 hover:bg-slate-50" onClick={()=> setF(prev=>({...prev, fee: defaultFeeFor(selectedKind)}))}>Авто</button>
              </div>
              {selectedKind && (selectedKind.percent ? (
                <div className="mt-1 text-xs text-amber-700">Правило: {Math.round(selectedKind.percent*100)}% від повного збитку — введіть суму вручну</div>
              ) : selectedKind.max==='full' ? (
                <div className="mt-1 text-xs text-amber-700">Можливе повне відшкодування залежно від пошкодження</div>
              ) : selectedKind.range ? (
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <div className="mb-1 text-slate-500">Фото</div>
              <input className="w-full rounded-xl border px-3 py-2" type="file" accept="image/*" onChange={(e)=>{
                const file = e.target.files?.[0]
                setF({...f, photoName: file? file.name : ''})
              }} />
              {f.photoName && <div className="mt-1 text-xs text-slate-500">Обрано: {f.photoName}</div>}
            </div>
            <div>
              <div className="mb-1 text-slate-500">Нотатка</div>
              <input className="w-full rounded-xl border px-3 py-2" value={f.note} onChange={e=>setF({...f, note:e.target.value})} placeholder="Опишіть проблему…" />
            </div>
          </div>
          <div className="flex justify-end gap-2">
            <Pill tone='slate' onClick={onClose}>Скасувати</Pill>
            <Pill tone='green' onClick={()=>{
              if(!f.kindCode){ alert('Оберіть тип пошкодження'); return }
              const saved = { id:'fd-'+Math.floor(Math.random()*90000+100), at: nowISO(), kind:f.kindCode, category:f.category, severity:f.severity, note:f.note, fee:f.fee, photoName:f.photoName }
              onSave(saved)
            }}>Зберегти</Pill>
          </div>
        </div>
        ...
        {item.findings && item.findings.length > 0 && (
          <div className="mt-4 border-t pt-4">
            <div className="font-semibold mb-2">Історія пошкоджень:</div>
            <ul className="space-y-1 text-sm">
              {item.findings.map(d=> (
                <li key={d.id} className="border-b pb-1">
                  <Badge tone='amber'>{d.kind}</Badge> · <Badge tone={d.severity==='high'?'red':d.severity==='mid'?'amber':'slate'}>{d.severity}</Badge> · ₴{d.fee} · {d.note || '—'}
                  <div className="text-slate-400 text-xs mt-0.5">{d.at?.slice(0,16)} {d.photoName? `· 📷 ${d.photoName}`:''}</div>
                </li>
              ))}
            </ul>
          </div>
