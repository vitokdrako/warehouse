...
def sync_products_incremental():
    """Sync only new products (one-time import, no updates)"""
    log("📦 Syncing products (new only)...")
    try:
        oc = mysql.connector.connect(**OC)
        rh = mysql.connector.connect(**RH)
...
def sync_orders_from_opencart():
    ...
    Після імпорту міняє статус в OC на 29 (Відправлено в РХ)
    """
    ...
    try:
        ...
        for order in new_orders:
            ...
            try:
                ...
                # ✅ Змінити статус в OpenCart на 29 (Відправлено в РХ)
                oc_cur.execute("""
                    UPDATE oc_order 
...
def main():
    ...
    # 🖼️ Download images for products without local photos
    img_count = sync_product_images()
    
    # ✅ Push quantities RH -> OC (замість старого OC -> RH)
